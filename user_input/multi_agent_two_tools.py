...
def create_agent(customer_id: str, rules_text: str = None):
    ...
    input_agent = Agent(
        role="Fetch Agent",
        goal=f"Use the data fetcher tool to gather the information about {customer_id}",
        backstory=(
    ...
    rules_agent = Agent(
        role = "Rules and decision agent",
        goal = "Use the rules provider tool to generate the final output",
        backstory = (
            "Reads business rules from the rules provider tool and uses it to evaluate rules."
            "The agent must return only the required JSON"
        ),
        tools = [rules_provider],
        allow_delegation = False,
        verbose = True,
        llm = ollm
    )
    return input_agent, rules_agent



def create_task(customer_id: str, rules_text: str = None):
    input_agent, rules_agent = create_agent(customer_id, rules_text=DEFAULT_RULES_TEXT)

    input_agent_task = Task(
        description=f"Use the data fetcher tool to gather information about the {customer_id}",
        expected_output= "A json object of fetched customer id",
        agent=input_agent
    )

    rules_agent_task = Task(
        description = " generate the final output based on the rools provided by rules provider tool, apply the rules on the input data give by the input agent",
        expected_output= "Final decision, and reason for the decision as a json object ",
        agent=rules_agent
    )
    
    return input_agent_task, rules_agent_task
...
def main(user_prompt):  
    ...
    if result is None:
        ...
        customer_id = result
        input_agent, rules_agent = create_agent(customer_id)
        input_agent_task, rules_agent_task= create_task(customer_id)


        crew = Crew(
            agents=[input_agent, rules_agent],
            tasks=[input_agent_task, rules_agent_task],
            verbose=True
        )
